...
# ---- quarantine_node (Tier 3) ----------------------------------------------


def test_ingest_enqueues_a_quarantine_node_command_when_mapped(isolated_manager, isolated_db):
    client.post(
    ...
    commands = client.get(f"/api/runtime-clusters/{cluster_id}/commands?token={token}").json()
    assert len(commands) == 1
    assert commands[0]["action"] == "quarantine_node"
    assert commands[0]["status"] == "pending"


def test_release_rejects_an_applied_quarantine_node_command(isolated_manager, isolated_db):
    client.post(
        "/api/response-rules", json={"rule_id": "Read sensitive file untrusted", "action": "quarantine_node"}
    )
    cluster_id, token = _create_cluster()
