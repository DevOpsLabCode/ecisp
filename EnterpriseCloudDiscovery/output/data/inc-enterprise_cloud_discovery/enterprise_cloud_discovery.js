...
function updateNavbar(path) {
    ...
    if (path === '') {
        $('#enterprise_cloud_discovery_navbar').addClass('active')
...
/**
 * Get the format of the results that Enterprise Cloud Discovery Engine is reading from
 */
function getFormat() {
...
function loadMetadata() {
    if (getFormat() === resultFormats.json) {
        runResults = getEnterpriseCloudDiscoveryResultsJson()
    } else if (getFormat() === resultFormats.sqlite) {
...
/**
 * Show About Enterprise Cloud Discovery Engine modal
 */
function showAbout() {
    $('#modal-container').html(about_enterprise_cloud_discovery_template())
    $('#modal-container').modal()
}
