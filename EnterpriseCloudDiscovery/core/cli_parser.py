import argparse
from EnterpriseCloudDiscovery import __version__


class EnterpriseCloudDiscoveryArgumentParser:

    def __init__(self):
        self.parser = argparse.ArgumentParser(epilog='To get addtional help on a specific provider run: enterprise-cloud-discovery.py {provider} -h')
        ...
        self.parser.add_argument('-v', '--version',
                                 action='version',
                                 version=f'Enterprise Cloud Discovery Engine {__version__}')

        self.common_providers_args_parser = argparse.ArgumentParser(add_help=False)

        self.subparsers = self.parser.add_subparsers(title="The provider you want to run enterprise-cloud-discovery against",
                                                     dest="provider")
    ...
    def _init_kubernetes_parser(self):
        ...
        kubernetes_scope.add_argument('-c',
                                      ...
                                      default=None,
                                      choices=['aks', 'eks', 'gke'],
                                      help='Cluster contexts to scan. If no cloud provider is specified, EnterpriseCloudDiscovery will use the default Kubernetes configuration.')
    ...
    def _init_common_args_parser(self):
        ...
        parser.add_argument('--max-workers',
                            ...
                            type=int,
                            default=10,
                            help='Maximum number of threads (workers) used by Enterprise Cloud Discovery Engine (default is 10)')
